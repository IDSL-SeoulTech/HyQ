...
from timm.layers import ClassifierHead, BatchNormAct2d, DropPath, AvgPool2dSame, \
    create_conv2d, get_act_layer, get_norm_act_layer, get_attn, make_divisible, to_2tuple, EvoNorm2dS0a #, ConvNormAct
...
from .layers_quant import Shift_Swish
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(
        ...
        if FQcfg.OLC:
            self.conv1_1x1_olc = QOlc(in_chans= in_chs)
            self.conv1_1x1_olc_qact = QAct(
                        bit_type=FQcfg.BIT_TYPE_A,
                        calibration_mode=FQcfg.CALIBRATION_MODE_A,
                        observer_str=FQcfg.OBSERVER_A_OLC,
                        quantizer_str=FQcfg.QUANTIZER_A)
        else:
            self.conv1_1x1_olc = QOlc(in_chans= in_chs)
            self.conv1_1x1_olc_qact = QAct(
        ...
        self.conv1_1x1.conv = QConv2d(in_chs,
            mid_chs,
            kernel_size=1,
            bias=False,
        ...
        # self.conv2_kxk_olc = QOlc(in_chans= mid_chs)
        # self.conv2_kxk_olc_qact = QAct(
        #     bit_type=FQcfg.BIT_TYPE_A,
        #     calibration_mode=FQcfg.CALIBRATION_MODE_A,
        #     observer_str=FQcfg.OBSERVER_A_OLC,
        #     quantizer_str=FQcfg.QUANTIZER_A)
        ...
        self.conv2_kxk = layers.conv_norm_act(
            ...
            stride=stride, dilation=dilation[0], groups=groups, drop_layer=drop_block,apply_act=False #apply_act=True 
        )

        ## BJ : APPLY shift_swish
        if(FQcfg.shift_swish):
            self.qact2_kxk_shift = QAct(
                bit_type=FQcfg.BIT_TYPE_A,
                calibration_mode=FQcfg.CALIBRATION_MODE_A,
                observer_str=FQcfg.OBSERVER_A,
                quantizer_str=FQcfg.QUANTIZER_A)
                    
            self.conv2_kxk_act = nn.SiLU(inplace=True)
            self.conv2_kxk_shift_act = Shift_Swish()

        else:
            self.conv2_kxk_act = nn.SiLU(inplace=True)


        self.conv2_kxk.conv = QConv2d(mid_chs,
            mid_chs,
        ...
        if FQcfg.OLC:
            self.conv3_1x1_olc = QOlc(in_chans= mid_chs)
        ...
        self.conv3_1x1.conv = QConv2d(mid_chs,
            out_chs,
            kernel_size=1,
            bias=False,
            bit_type=FQcfg.BIT_TYPE_W,
    ...
    def forward(self, x):
        ...
        #x = self.qact_bt_input(x)
        
        
        shortcut = x
        
        shortcut = self.qact_shortcut(shortcut) # first here: 67.464 // last: 67.472 
        
        ## Quant Conv1_1x1 Input
        x = self.conv1_1x1_olc(x)
        ...
        if(self.cfg.shift_swish):
            if((self.qact2_kxk_shift.calibrate == True)|(self.qact2_kxk_shift.olc_calibrate==True)):
                x = self.qact2_kxk_shift(x)
                x = self.conv2_kxk_act(x)
                
            else:
                x = self.qact2_kxk_shift.quantizer.quant(x)
                scale,zero_point = self.qact2_kxk_shift.observer.get_quantization_params(x)
                x_div,s_div = self.conv2_kxk_shift_act((x-zero_point),scale,32)
        ...
        ## Quant Conv3_1x1 Input
        x = self.conv3_1x1_olc(x)
...
def create_block(block: Union[str, nn.Module], **kwargs):
    ...
    # print('block: ', block)
    # print('_block_registry[block]: ', _block_registry[block])
...
def update_block_kwargs(block_kwargs: Dict[str, Any], block_cfg: ByoBlockCfg, model_cfg: ByoModelCfg, FQcfg=None):
    ...
    if FQcfg is not None:
        # print('FQcfg is not None!')
        # block_kwargs.update(override_kwargs(block_cfg.block_kwargs, FQcfg))
        block_kwargs['FQcfg'] = FQcfg
...
def create_byob_stages(
    ...
    for stage_idx, stage_block_cfgs in enumerate(block_cfgs):
        ...
        for block_idx, block_cfg in enumerate(stage_block_cfgs):
            ...
            block_kwargs_fn(block_kwargs, block_cfg=block_cfg, model_cfg=cfg, FQcfg=FQcfg)
...
class ByobNet(nn.Module):
    ...
    def __init__(
        ...
        ## NJ stem conv
        self.stem.conv = QConv2d(in_chans,
            stem_chs,
            kernel_size=3,
            stride=2,
            bias=False,
        ...
        ## BJ SHIFT_SWISH
        # if(FQcfg.shift_swish):
        #     self.stem.bn.act=nn.Identity()
        #     self.qact_shift_stem = QAct(
        #             bit_type=FQcfg.BIT_TYPE_U16,
        #             calibration_mode=FQcfg.CALIBRATION_MODE_A,
        #             observer_str=FQcfg.OBSERVER_A,
        #             quantizer_str=FQcfg.QUANTIZER_A)
        
        #     self.stem_act = nn.SiLU()
        #     self.stem_shift_act = Shift_Swish()
        ...
        if cfg.num_features:
            ...
            self.final_conv = layers.conv_norm_act(prev_chs, self.num_features, 1,apply_act=False) #apply_act= True
        
            ## NJ final conv
            self.final_conv.conv = QConv2d(prev_chs,
                self.num_features,
                kernel_size=1,
                bias=False,
                bit_type=FQcfg.BIT_TYPE_W,
            ...
            #BJ SHIFT SWISH
            if(FQcfg.shift_swish):
                self.final_conv_qact = QAct(
                        bit_type=FQcfg.BIT_TYPE_A,
                        calibration_mode=FQcfg.CALIBRATION_MODE_A,
                        observer_str=FQcfg.OBSERVER_A,
                        quantizer_str=FQcfg.QUANTIZER_A)
                        
                self.final_conv_act = nn.SiLU(inplace=True)
                self.final_conv_shift_act = Shift_Swish()
            else :
                self.final_conv_act = nn.SiLU(inplace=True)
    ...
    @torch.jit.ignore
    def model_open_olc_calibrate(self):
        for m in self.modules():
            if type(m) in [QConv2d, QLinear, QAct]:
                m.olc_calibrate = True
    @torch.jit.ignore
    def model_close_olc_calibrate(self):
        for m in self.modules():
            if type(m) in [QConv2d, QLinear, QAct]:
    ...
    def forward_features(self, x):
        ...
        ###BJ : apply shift-swish
        if(self.FQcfg.shift_swish):
            if((self.qact_shift_stem.calibrate == True)|(self.qact_shift_stem.olc_calibrate==True)):
                x = self.qact_shift_stem(x)
          
                x = self.stem_act(x)
            else:
                x = self.qact_shift_stem.quantizer.quant(x)
                scale,zero_point = self.qact_shift_stem.observer.get_quantization_params(x)
                x_div,s_div = self.stem_shift_act((x-zero_point),scale,32)
                x=(x-zero_point)*x_div*s_div*scale
        else:
            pass
        ...
        ## BJ  : apply shift-swish
        if(self.FQcfg.shift_swish):
            if((self.final_conv_qact.calibrate == True)|(self.final_conv_qact.olc_calibrate == True)):
                x = self.final_conv_qact(x)
                x = self.final_conv_act(x)
            else:
                x = self.final_conv_qact.quantizer.quant(x)
                scale,zero_point = self.final_conv_qact.observer.get_quantization_params(x)
                x_div,s_div = self.final_conv_shift_act((x-zero_point),scale,32)
                x=(x-zero_point)*x_div*s_div*scale                    
        else:
            x = self.final_conv_act(x)
