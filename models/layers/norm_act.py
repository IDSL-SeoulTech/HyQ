...
class BatchNormAct2d(nn.BatchNorm2d):
    ...
    def forward(self, x):
        ...
        # 80
        # 320
        # if type(self.act) in [nn.SiLU] and self.num_features==320:
            
        #     # x = x * (0.0008315*(x**4) - 0.003404**(x**3) - 0.03827*(x**2) + 0.2754*x + 0.4965)
            
        #     sign = torch.sign(x)
        #     i_sigmoid = 1.2074506214466665*(x*sign)**3 -0.0036186215072003293 * (x*sign)**2 + 0.007351673003716259*(x*sign) + 0.9925934703142986
        #     x = sign * i_sigmoid
        ...
        #     # if self.num_features == 16 and self.iter == 0:

                
        #     #     smx_file = 'swish_output.txt'
        #     #     y = y2.cpu().detach().numpy()
        #     #     # Convert 4D array to 1D array
        #     #     y_1d = y.flatten()
        #     #     np.savetxt(smx_file, y_1d, fmt='%.8f')

        #     #     self.iter = self.iter + 1
            
        # elif self.act == nn.Identity():
        #     x = x
        # else:
        #     x = self.act(x)
