...
class Attention(nn.Module):
    ...
    def __init__(self,
        ...
        if window_size is not None:
            self.scale = float(self.num_heads)**-0.5
            self.attn_area: int = window_size[0] * window_size[1]
            # Define a parameter table of relative position bias, shape: 2*Wh-1 * 2*Ww-1, nH
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), self.head_dim))# self.num_heads))
            
            # Get pair-wise relative position index for each token inside the window
            self.register_buffer("relative_position_index", get_relative_position_index(window_size[0], window_size[1]))
    ...
    def _get_relative_positional_bias(
        ...
        """ Returns the relative positional bias. for MOAT
        Returns:
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape
        x = self.qkv(x)
        x = self.qact1(x) ## 67.660
        
        
        if self.window_size is not None:
            qkv = x.reshape(B, N, 3, self.head_dim, -1).permute(2, 0, 3, 1, 4)  # (BN33)
            q, k, v = qkv.unbind(0)
        else:
...
class Block(nn.Module):
    ...
    def forward(self, x, last_quantizer=None):
        x = self.qact2(x + self.drop_path(
            self.attn(
                self.qact1(
                    self.norm1(x, last_quantizer, self.qact1.quantizer)))))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.qact_embed = QAct(quant=quant,
                               ...
                               observer_str=cfg.OBSERVER_A,
                               quantizer_str=cfg.QUANTIZER_A)
        self.qact_pos = QAct(quant=False,
                             calibrate=calibrate,
                             bit_type=cfg.BIT_TYPE_A,
                             calibration_mode=cfg.CALIBRATION_MODE_A,
                             observer_str=cfg.OBSERVER_A,
    ...
    def model_quant(self):
        for m in self.modules():
            if type(m) in [QConv2d, QLinear, QAct, QIntSoftmax, QOlc]:
                m.quant = True
    ...
    def model_open_smooth(self):
        for m in self.modules():
            if type(m) in [QOlc]:
                m.smooth = True
    def model_close_smooth(self):
        for m in self.modules():
            if type(m) in [QOlc]:
                m.smooth = False
