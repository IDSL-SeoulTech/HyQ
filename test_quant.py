...
from models import *
# from models.build_moat import build_moat
...
parser.add_argument('--shift_swish', default=False, action='store_true') 
parser.add_argument('--QADS', default=False, action='store_true')
parser.add_argument('--bn_tuning', default=False, action='store_true')
...
def main():
    ...
    cfg = Config(args.ptf, args.lis, args.QADS, args.quant_method, shift_swish = args.shift_swish)
    #print('Config: ', dir(cfg), '\n')
    if 'mobilevit' in args.model:
        model_name = args.model + '.cvnets_in1k'
        model = create_model(
            model_name,
            #args.model,
    ...
    train_transform = build_transform(input_size=input_size, mean=mean, std=std, crop_pct=crop_pct)
    val_transform = build_transform(input_size=input_size, mean=mean, std=std, crop_pct=crop_pct)
    ...
    if args.quant:
        ...
        olc_dataset = datasets.ImageFolder(traindir, train_transform)
        olc_loader = torch.utils.data.DataLoader(
            olc_dataset,
            batch_size=100,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        if args.QADS:
            ...
            for i, (data, target) in enumerate(train_loader):
                if i == args.calib_iter:
                    break
                data = data.to(device)
                image_list_a.append(data)
                
            print('Assign Olc factor...')
            with torch.no_grad():
                model.model_open_olc_calibrate()
                for i, image in enumerate(image_list_a):
                    if i == len(image_list_a) - 1:
                        # This is used for OMSE method to
                        # calculate minimum quantization error
                        model.model_open_last_calibrate()
                    output = model(image)
                assign_ptf(model, model_size, cfg)
                model.model_close_olc_calibrate()
